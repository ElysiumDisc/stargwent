...
void main() {
    ...
    if (norm < 1.05 && norm > 0.55) {
        ...
        // Hex color — cyan-blue energy
        vec3 hex_color = mix(vec3(0.3, 0.7, 1.0), vec3(0.5, 0.9, 1.0),
    ...
    vec3 rim_color = mix(vec3(0.4, 0.8, 1.0), vec3(0.7, 0.95, 1.0), pulse);
    color.rgb += rim_color * rim * 0.45;

    // ---- Faint inner glow ----
    if (norm < 1.0) {
        float inner = (1.0 - norm) * shield_pct * 0.06;
        color.rgb += vec3(0.2, 0.5, 0.8) * inner;
    }
...
class ShieldBubblePass(ShaderPass):
    ...
    def update_shield(self, center_uv, radius_uv, pct, t, screen_size=None):
