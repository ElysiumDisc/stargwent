...
def draw_board(surface, game, selected_card, dragging_card=None):
    ...
    weather_x = SCREEN_WIDTH - CARD_WIDTH - 20  # Right side of screen
    for weather_entry in game.weather_cards_on_board:
        if isinstance(weather_entry, tuple):
            weather_card, _owner = weather_entry
        else:
            weather_card = weather_entry
        ability = weather_card.ability or ""
        
        # Determine which row(s) this weather affects
        affected_rows = []
        if "Ice Planet Hazard" in ability:
            affected_rows = ["close"]
        elif "Nebula Interference" in ability:
            affected_rows = ["ranged"]
        elif "Asteroid Storm" in ability:
            affected_rows = ["siege"]
        elif "Wormhole Stabilization" in ability:
            # Clear weather card - show on all rows or center
            affected_rows = ["close", "ranged", "siege"]
        elif "Electromagnetic Pulse" in ability:
            affected_rows = ["close", "ranged", "siege"]
        
        # Draw the weather card on the right side of each affected row
        # Weather affects BOTH players' rows
        for row_name in affected_rows:
            if row_name in PLAYER_ROW_RECTS:
                row_rect = PLAYER_ROW_RECTS[row_name]
                weather_y = row_rect.top + card_y_offset
                draw_card(surface, weather_card, weather_x, weather_y)
                # Only draw once per weather card (use player1's row as reference)
                break
...
def main():
    ...
    while running:
        ...
        if game.current_player == game.player2 and game.game_state == "playing":
            ...
            if ai_result == "thinking_done":
                ...
                if ai_card_to_play and ai_row_to_play:
                    ...
                    # Trigger stargate activation effect
                    effect_x = SCREEN_WIDTH // 2
                    effect_y = OPPONENT_ROW_RECTS[ai_row_to_play].centery if ai_row_to_play in OPPONENT_ROW_RECTS else SCREEN_HEIGHT // 4
        ...
        if game.game_state == "mulligan":
            ...
            ai_ability_y = 20
            draw_leader_ability_box(screen, game.player2, ai_ability_x, ai_ability_y, 
            ...
            draw_leader_ability_box(screen, game.player1, player_ability_x, player_ability_y, 
